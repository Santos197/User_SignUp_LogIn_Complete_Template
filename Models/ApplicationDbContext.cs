...
public class ApplicationDbContext : IdentityDbContext<IdentityUser> //DbContext
{
    ...
    private static LambdaExpression MakeEqualityComparison<T>(IReadOnlyTypeBase entityType, string property,Expression value)
    {
        ...
        foreach (var entry in ChangeTracker.Entries<BaseModel>().ToList())
        {
            switch (entry.State)
            {
                case EntityState.Added:
                    ...
                    break;
                case EntityState.Modified when entry.Entity.IsDeleted && !entry.OriginalValues.GetValue<bool>(nameof(entry.Entity.IsDeleted)):
